...
public class miniadapter extends RecyclerView.Adapter<miniadapter.ViewHolder>{
    ...
    Context context;
    List<minimodel> mini_list;

    public miniadapter(Context context,List<minimodel> mini_list){
        this.context = context;
        this.mini_list = mini_list;
    }
    @NonNull
    @Override
    public miniadapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.mini_item_layout,parent,false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull miniadapter.ViewHolder holder, int position) {
        if(mini_list!=null && mini_list.size()>0){
            minimodel model = mini_list.get(position);
